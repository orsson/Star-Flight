...
import pl.starflight.SFBackgrund;
import pl.starflight.SFEngine;


import android.opengl.GLSurfaceView.Renderer;

public class SFGameRenderer implements Renderer{
	private SFBackgrund background = new SFBackgrund();
	private SFBackgrund background2 = new SFBackgrund();
	private SFGoodGuy player1 = new SFGoodGuy();
	private SFEnemy[] enemies = new SFEnemy[SFEngine.TOTAL_INTERCEPTORS + SFEngine.TOTAL_SCOUTS + SFEngine.TOTAL_WARPSHIPS - 1];
	private SFTextures textureLoader;
	private int[] spriteSheets = new int[2];
	private SFWeapon[] playerFire = new SFWeapon[8];
	...
	private void initializeInterceptors(){
		for (int x = 0; x<SFEngine.TOTAL_INTERCEPTORS -1 ; x++){
			SFEnemy interceptor = new SFEnemy(SFEngine.TYPE_INCEPTOR, SFEngine.ATTACK_RANDOM);
	...
	private void initializeWarships(){
		for (int x = SFEngine.TOTAL_INTERCEPTORS + SFEngine.TOTAL_SCOUTS -1; x<SFEngine.TOTAL_INTERCEPTORS + SFEngine.TOTAL_SCOUTS + SFEngine.TOTAL_WARPSHIPS -1; x++){
			SFEnemy interceptor = new SFEnemy(SFEngine.TYPE_WARPSHIP, SFEngine.ATTACK_RANDOM);
			enemies[x] = interceptor;
		}
	}
	...
	private void moveEnemy(GL10 gl){
		for (int x = 0; x < SFEngine.TOTAL_INTERCEPTORS + SFEngine.TOTAL_SCOUTS + SFEngine.TOTAL_WARPSHIPS - 1; x++){
			if (!enemies[x].isDestroyed){
				Random randomPos = new Random();
				switch (enemies[x].enemyType){
				case SFEngine.TYPE_INCEPTOR:
					if (enemies[x].posY < 0){
				...
				case SFEngine.TYPE_WARPSHIP:
					...
					if (enemies[x].posY >= 3){
						enemies[x].posY -= SFEngine.WARPSHIP_SPEED;
			
					}else{
						if (!enemies[x].isLockedOn){
							enemies[x].lockOnPosX = randomPos.nextFloat() * 3;
							enemies[x].isLockedOn = true;
							enemies[x].incrementXToTarget =  (float) ((enemies[x].lockOnPosX - enemies[x].posX )/ (enemies[x].posY / (SFEngine.WARPSHIP_SPEED  * 4)));
						}
						enemies[x].posY -= (SFEngine.WARPSHIP_SPEED  * 2);
						enemies[x].posX += enemies[x].incrementXToTarget; 
